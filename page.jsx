...
export default function Home() {
  ...
  const [form, setForm] = useState({ name: "", company: "", country: "", whatsapp: "" });

  const handleSubmit = (e) => {
    e.preventDefault();
    const message = `New Lead:%0AName: ${form.name}%0ACompany: ${form.company}%0ACountry: ${form.country}%0AWhatsApp: ${form.whatsapp}`;
  ...
  return (
    ...
    >
      ...
      <section className="px-6 py-28 max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-center">
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-5xl font-bold">
            Get More International Patients — Without Risk
          </h1>
          <p className="mt-6 text-lg text-gray-600">
            We connect you with verified clinics and handle operations so you can scale your agency globally.
          </p>
          <div className="mt-8 flex gap-4">
            <a href="#contact" className="bg-black text-white px-6 py-3 rounded-2xl">Start Partnership</a>
          </div>
        </motion.div>
      ...
      <section className="py-24 max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-center">
        ...
        <div>
          ...
          <p className="mt-4 text-gray-600">
            We are a medical tourism agency network connecting international partners with premium clinics in Turkey. Our system eliminates risk, improves patient satisfaction and helps agencies scale sustainably.
          </p>
        </div>
      </section>
        ...
        <div className="grid md:grid-cols-3 gap-10 mt-14">
          {[{
            ...
            <div key={i} className="rounded-2xl overflow-hidden shadow-lg group">
              <div className="h-56 bg-cover bg-center group-hover:scale-105 transition" style={{backgroundImage:`url(${item.img})`}}></div>
      ...
      <section className="py-24 bg-white/70 backdrop-blur">
        <div className="max-w-6xl mx-auto text-center px-6">
          ...
          <div className="grid md:grid-cols-3 gap-10 mt-12">
            {["High commission opportunities","Reliable clinic network","Fast patient coordination"].map((item,i)=>(
      ...
      <section id="contact" className="py-24 px-6 max-w-xl mx-auto">
        <div className="bg-white/90 p-10 rounded-3xl shadow-xl">
          ...
          <form onSubmit={handleSubmit} className="mt-8 grid gap-4">
            <input required placeholder="Full Name" className="border p-3 rounded-xl" onChange={(e)=>setForm({...form,name:e.target.value})}/>
            <input required placeholder="Company" className="border p-3 rounded-xl" onChange={(e)=>setForm({...form,company:e.target.value})}/>
            <input required placeholder="Country" className="border p-3 rounded-xl" onChange={(e)=>setForm({...form,country:e.target.value})}/>
            <input required placeholder="WhatsApp" className="border p-3 rounded-xl" onChange={(e)=>setForm({...form,whatsapp:e.target.value})}/>
      ...
      {/* FLOAT CTA */}
      <a href="#contact" className="fixed bottom-6 left-6 bg-black text-white px-6 py-3 rounded-full shadow">Get Clients</a>
      <a href="https://wa.me/905000000000" className="fixed bottom-6 right-6 bg-green-500 text-white px-5 py-3 rounded-full shadow">WhatsApp</a>

      <footer className="py-10 text-center text-gray-500">
        © {new Date().getFullYear()} NewLifeHealth
      </footer>
    </div>
  );
}
